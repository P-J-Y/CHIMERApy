...
f171=findfile(temp+'/*171*.f*')
f193=findfile(temp+'/*193*.f*')
f211=findfile(temp+'/*211*.f*')
...
r = (s[1]/2.0)-450 ;	solar radius [pixel], ~975"
...
; what is ind? -read_sdo,fil,ind,data, /use_shared_lib
; IDL> print,ind.wavelnth
;          171         193         211
ind[*]=ind[reord]
data[*,*,*]=data[*,*,reord]
...
index2map,ind,data,map ;	index2map: Make an image map from index/data pair
rs=(map[1].rsun)[0]	;	solar radius 975"

if ind[1].cdelt1 gt 1 then begin
; IDL> help,ind[1].cdelt1,ind[1].cdelt2
; <Expression>    DOUBLE    =       0.60000002
; <Expression>    DOUBLE    =       0.60000002
	ind.cdelt1=ind.cdelt1/4
	ind.cdelt2=ind.cdelt2/4
; IDL> help,ind[1].crpix1,ind[1].crpix2
; <Expression>    DOUBLE    =        2048.5000
; <Expression>    DOUBLE    =        2048.5000
...
for i =0L, n_elements(w)-1 do begin
	tem[dat0[w[i]],dat1[w[i]]]=tem[dat0[w[i]],dat1[w[i]]]+1 ;	171vs193
	tmp[dat0[w[i]],dat2[w[i]]]=tmp[dat0[w[i]],dat2[w[i]]]+1 ;	171vs211
	tep[dat1[w[i]],dat2[w[i]]]=tep[dat1[w[i]],dat2[w[i]]]+1 ;	193vs211
	;只有满足上面条件（强度大于4000，在日面上）的点，值才为+1，其余为0
endfor
...
;	这一步很重要，其实做了一个logical conjunction
